...
for _ in range(n):
    ...
    center = center + v * m
    M += m
print(f'Сenter of mass coordinates: {center + 1 / M}')
    ...
    for i in range(n):
        for j in range(n):
            for k in range(j + 1, n):
                v1 = Vector(*Set_of_vectors[j]) + Vector(*Set_of_vectors[i])
                v2 = Vector(*Set_of_vectors[k]) + Vector(*Set_of_vectors[i])
